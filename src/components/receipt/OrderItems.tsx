          ...
          <div className="bg-white border border-teal-100 rounded-xl p-3 sm:p-4 space-y-3">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center gap-3 flex-wrap">
              ...
              <div className="flex items-center gap-2.5 min-w-0">
                <span className="text-xs font-bold text-slate-900 uppercase tracking-wider whitespace-nowrap shrink-0">
                  {t('orderType')}
                </span>
              ...
              <div className="hidden sm:block">
                <button
                  ...
                  className="flex items-center gap-2 text-xs font-bold text-slate-900 hover:text-black transition-colors py-2 px-3 bg-white border border-slate-300 rounded-xl hover:bg-slate-50"
                >
              ...
              <label className="flex items-center gap-3 py-2 px-3 sm:px-4 bg-white hover:bg-teal-50 border border-teal-100 hover:border-teal-300 rounded-xl cursor-pointer transition-colors select-none">
                ...
                />
                <span className="text-xs font-bold text-slate-900 whitespace-nowrap">{t('manualAdditionalCosts')}</span>
                {manualAdditionalCostsEnabled && (
            ...
            <div className="sm:hidden pt-1">
              <button
                onClick={() => setShowFilters(v => !v)}
                className="flex items-center gap-2 text-xs font-bold text-slate-900 hover:text-black transition-colors"
              >
        ...
        <div className="p-4 sm:p-6 space-y-3">
          {items.map((item, index) => {
            ...
            return (
              <div
                key={item.id}
                className={`item-card slide-in bg-white border rounded-2xl overflow-hidden ${isOOS ? 'border-red-200' : 'border-blue-400'}`}
              >
                {/* Card Header Bar */}
